...
with st.sidebar:
    ...
    tarifa_input = st.number_input("Tarifa base (ARS)", value=6_000, step=500, key="tarifa")
    st.caption("📌 Regla de ingresos: Año 1 → tránsito arranque (UTEQs base), genera ingresos. Año 2+ → crece a tasa base 3% + Δ abajo.")
    delta_trafico = st.slider(
        "Δ tasa crecimiento anual Año 2+ (±pp sobre base 3%)",
        min_value=-3.0, max_value=5.0, value=0.0, step=0.25,
        format="%.2f pp", key="trafico",
    )
    ...
    al_ganancias = st.slider(
        "Ganancias (%)", min_value=0.0, max_value=55.0,
        value=AL_GANANCIAS_BASE * 100, step=0.5,
        format="%.1f %%", key="gan",
    ) / 100
    al_ib = st.slider(
        "Ingresos Brutos (%)", min_value=0.0, max_value=10.0,
        value=AL_IB_BASE * 100, step=0.1,
        format="%.1f %%", key="ib",
    ) / 100
    al_municipal = st.slider(
        "Tasas Municipales (%)", min_value=0.0, max_value=3.0,
        value=AL_MUNICIPAL_BASE * 100, step=0.1,
        format="%.1f %%", key="mun",
    ) / 100
    al_sellos = st.slider(
        "Impuesto de Sellos (%)", min_value=0.0, max_value=5.0,
        value=AL_SELLOS_BASE * 100, step=0.1,
        format="%.1f %%", key="sel",
    ) / 100
    al_dbcr = st.slider(
        "Débitos y Créditos Bancarios (%)", min_value=0.0, max_value=5.0,
        value=AL_DBCR_BASE * 100, step=0.1,
        format="%.1f %%", key="dbcr",
    ) / 100
    al_iva_peaje = st.slider(
        "IVA sobre peaje (%)", min_value=0.0, max_value=27.0,
        value=AL_IVA_BASE * 100, step=0.5,
        format="%.1f %%", key="iva",
    ) / 100
    ...
    tasa_van = st.slider(
        "Tasa de descuento (%)", min_value=5.0, max_value=25.0,
        value=10.0, step=0.5,
        format="%.1f %%", key="tasa",
    ) / 100
...
# ─────────────────────────────────────────────────────────────
# TAB 1 – FLUJO  (layout 2×2 igual que Caucete)
# ─────────────────────────────────────────────────────────────
_AX = dict(gridcolor="#252f45", zeroline=False, color="#c5cdd8",
           showline=False, tickfont=dict(color="#8a9bb0"))

def _fig_base(title, h=340):
    f = go.Figure()
    f.update_layout(
        title=dict(text=title, font=dict(color="#c5cdd8", size=13), x=0.5, xanchor="center"),
        plot_bgcolor="#0e1117", paper_bgcolor="#0e1117",
        font=dict(color="#c5cdd8", size=11),
        height=h, margin=dict(t=45, b=45, l=55, r=25),
        legend=dict(bgcolor="rgba(0,0,0,0)", font=dict(color="#8a9bb0", size=10),
...
with tab1:
    ...
    # ── TOP-LEFT: Flujo Neto Anual ────────────────────────────
    with col_l:
        f1 = _fig_base("Flujo Neto Anual  ($ MM ARS)")
        colors_bar = [C["pos"] if v >= 0 else C["neg"] for v in sc["flujo"]]
        f1.add_trace(go.Bar(
            x=YEARS_INT, y=sc["flujo"]/1e9,
            marker_color=colors_bar, opacity=0.9, name="Flujo neto",
            showlegend=False,
        ))
        f1.add_hline(y=0, line_color="#555", line_width=1)
        st.plotly_chart(f1, use_container_width=True)

    # ── TOP-RIGHT: Flujo Acumulado ────────────────────────────
    with col_r:
        f2 = _fig_base("Flujo Acumulado  ($ MM ARS)")
        ...
        f2.add_hline(y=0, line_color="#555", line_width=1, line_dash="dot")
        st.plotly_chart(f2, use_container_width=True)

    # ── BOTTOM-LEFT: Composición de Egresos ──────────────────
    with col_l:
        f3 = _fig_base("Composición de Egresos por año  ($ MM ARS)")
        cats_egr = [
            ("Egresos",   sc["total_egr"],          C["neg"]),
            ("Ingresos",  sc["total_ing"],           C["pos"]),
            ("Deuda LP",  sc["amort_deuda"],         C["pur"]),
            ("Impuestos", sc["total_imp"],           C["warn"]),
            ("OPEX",      sc["opex"],                "#4ecdc4"),
            ("CAPEX",     sc["capex"],               "#b98cf7"),
            ("Acumulado", sc["acum"],                "#4a9eff"),
            ("Flujo Neto",sc["flujo"],               "#f9c74f"),
        ]
        for name, vals, col in cats_egr:
    ...
    # ── BOTTOM-RIGHT: Ingresos vs Egresos ────────────────────
    with col_r:
        f4 = _fig_base("Ingresos vs Egresos  ($ MM ARS)")
        f4.add_trace(go.Scatter(
            x=YEARS_INT, y=sc["total_egr"]/1e9, name="Egresos",
            mode="lines+markers", line=dict(color=C["neg"], width=2.5),
            marker=dict(size=5),
        ))
        f4.add_trace(go.Scatter(
            x=YEARS_INT, y=sc["total_ing"]/1e9, name="Ingresos",
            mode="lines+markers", line=dict(color=C["pos"], width=2.5),
            marker=dict(size=5),
        ))
...
with tab2:
    ...
    shocks = {
        "Obras oblig. +20%":   dict(delta_capex_repav=delta_repav+0.20),
        "Obras oblig. –20%":   dict(delta_capex_repav=delta_repav-0.20),
        ...
        "Ganancias +10pp":     dict(al_ganancias=min(0.55, al_ganancias+0.10)),
    ...
    fig3.update_layout(**{**PL, "legend": dict(bgcolor="rgba(0,0,0,0)")}, height=400, margin=dict(t=40, b=50, l=60, r=20))
